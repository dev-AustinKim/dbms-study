...
SELECT * FROM PLAYER
WHERE WEIGHT BETWEEN 70 AND 80; -- 이상 이하만 된다. 초과미만은 안된다. 컬럼이 같을 때는 BETWEEN을 써도 된다.
...
/*PLAYER 테이블에서 HEIGHT가 170이상이고 WEIGHT가 80이상인 선수이름 */
SELECT *
FROM PLAYER
...
/*OR은 BETWEEN으로 묶고(이상/이하만 가능, 초과/미만은 안된다.) AND는 IN으로 묶는다.*/
...
/*AS뒤에 작성할 때 내가 원하는 컬럼명을 줄 때 띄어쓰기를 하면 오류가 난다. 이럴 때는 ""안에 써주면 오류가 나지 않는다.*/
...
SELECT PLAYER_NAME || '의 영어이름은 ' || E_PLAYER_NAME || '이다.' AS 자기소개 FROM PLAYER;

/*PLAYER_NAME의 포지션은 POSITION입니다.*/
SELECT PLAYER_NAME || '의 포지션은 ' || "POSITION"  || '입니다.' AS "포지션 소개" FROM PLAYER;
...
/* 집계 함수 : 결과는 무조건 1개*/
/*
 * ※ NULL은 포함시키지 않는다. BUT NVL을 사용하면 NULL도 포함한다.
 * ※ WHERE절에서 사용 불가.
 * 
 * 
 * 평군 : AVG()
...
SELECT "POSITION", COUNT("POSITION") AS "포지션 별 선수" FROM PLAYER
WHERE "POSITION" IS NOT NULL
GROUP BY "POSITION";

/*PLAYER 테이블에서 몸무게가 80 이상인 선수들의 평균키가 180이상인 포지션 검색*/

SELECT "POSITION", MIN(WEIGHT), AVG(HEIGHT) AS "평균키 180이상" FROM PLAYER
WHERE WEIGHT >= 80
GROUP BY "POSITION"
...
SELECT JOB_ID, AVG(SALARY) AS "평균월급 10000미만" FROM EMPLOYEES   
GROUP BY JOB_ID
...
SELECT "POSITION", AVG(HEIGHT), (SELECT AVG(HEIGHT) FROM PLAYER) "전체 평균 키" FROM PLAYER
WHERE "POSITION" IS NOT NULL
GROUP BY "POSITION";
...
/*
 * 여러 테이블에 흩어져 있는 정보 중
 * 시용자가 필요한 정보만 거져와서 가상의 테이블처럼 만들고 결과를 보여주는 것.
